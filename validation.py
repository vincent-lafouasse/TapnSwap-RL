...
class Optimizer:
    ...
    def __init__(
        ...
        assert (
            self.random_training or self.self_training
    ...
    def grid_search(self, n_epochs, n_games_test=100, freq_test=0, retrain=False):
        ...
        * Tournament ranking: TXT file
        ...
        for random_opponent in random_choices:
            ...
            for invert_opp in invert_choices:
                ...
                if invert_opp:
                    ...
                    new_opp = "vsSelf" * int(random_opponent) + "vsRandom" * (
                ...
                for epsilon in self.epsilon_values:
                    ...
                    if not retrain:
                        ...
                        rate_success = 0.99
                    if not (result[2] == result[3]) or not (
                        result[1] >= rate_success * result[2]
                    ):
                        print(
                            "At the end of training, the RL Agent has won "
        ...
        if len(problems) > 0:
            print(
                "Problem with training of the following agents: ",
                problems,
    ...
    def delete_temp(self, old_model, temp_model):
        ...
        if (old_model is not None) and (temp_model == old_model + "_temp"):
            ...
            results = compare_agents(
                agent1, agent2, n_games=10, time_limit=100, verbose=False
            )
    ...
    def tournament(self, change_opp=False):
        ...
        for idx1, player1 in enumerate(players):
            ...
            # 0: RANDOM | 1: SELF
            scores[idx1 + 3, 1] = int(training_way1 == "Self") + int(
                training_way1 == "SelfvsRandom"
            )
            # -1: nothing | 0: Random vs Self | 1: Self vs Random
            scores[idx1 + 3, 2] = -1 + (
                2 * int(training_way1 == "SelfvsRandom")
            ...
            for idx2, player2 in enumerate(players):
                ...
                scores[1, idx2 + 3] = int(training_way2 == "Self") + int(
                    training_way2 == "SelfvsRandom"
                )
                scores[2, idx2 + 3] = -1 + (
                    2 * int(training_way2 == "SelfvsRandom")
                    + int(training_way2 == "RandomvsSelf")
                )

                print("Current match:")
                print(
                    "Player1: epsilon = {}, trained vs {}".format(
                        epsilon1, training_way1
                    )
                )
                print(
                    "Player2: epsilon = {}, trained vs {}".format(
        ...
        # Save tournament
        np.savetxt(
            str("Models/results/" + self.tournament_name + ".csv"),
            scores,
            delimiter=",",
        )
        ...
        print(
            "Results of tournament are stored in {}.csv and {}.txt\n".format(
    ...
    def tournament_ranking(self, input_filename, output_filename):
        ...
        for player in indices_players_sorted:
            ...
            training_way = int(data[player + 3, 1]) * "Self" + (
                (1 - int(data[player + 3, 1])) * "Random"
            )
            # If this agent has been trained 2 times
            if int(data[player + 3, 2]) != -1:
                training_way = (int(data[player + 3, 2])) * "SelfvsRandom" + (
                    (1 - int(data[player + 3, 2])) * "RandomvsSelf"
                )
            ...
            with open("Models/results/" + output_filename + ".txt", "a") as f:
                f.write(
                    str(counter)
                    + ":\t"
                    + str(sum_scores[player])
                    + "\tepsilon = "
                    + str(epsilon)
                    + ", \ttrained vs "
                    + str(training_way)
                    + " \t"
                    + "\t" * (len(str(training_way)) < 7)
                    + str(n_epochs)
                    + "\tepochs\n"
                )
...
if __name__ == "__main__":
    ...
    optimizer.retrain_best_models(
        n_epochs=n_epochs, common_train_time=False, min_frac=0.3
    )
